document.addEventListener('DOMContentLoaded', function() {
  ...
  chrome.storage.sync.get(
    ['generalBlockedWords', 'linkBlockedWords', 'blockedUsers', 'enableUserBlocking', 'autoCollectUsers'], 
    function(result) {
  ...
  document.getElementById('saveButton').addEventListener('click', function() {
    const generalWords = document.getElementById('generalBlockedWords').value
      .split('\n')
      .filter(word => word.trim() !== '')
      .map(word => word.trim().toLowerCase());
      
    const linkWords = document.getElementById('linkBlockedWords').value
      .split('\n')
      .filter(word => word.trim() !== '')
      .map(word => word.trim().toLowerCase());
    
    const blockedUsers = document.getElementById('blockedUsers').value
      .split('\n')
      .filter(user => user.trim() !== '')
      .map(user => user.trim().toLowerCase());
    
    const enableUserBlocking = document.getElementById('enableUserBlocking').checked;
    ...
    chrome.storage.sync.set({
      ...
      showMessage('Settings saved! Refresh Twitter to see changes.', 'green');
    });
  });
